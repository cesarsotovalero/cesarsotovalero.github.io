...
The first is the default, and it is used by the [Java Hotspot Virtual Machine](https://www.oracle.com/java/technologies/javase/vmoptions-jsp.html) to translate bytecode into machine code at runtime.
The latter is supported by the novel [GraalVM](https://www.graalvm.org/) compiler and allows statically compiling bytecode directly into machine code.
In this post, I'll explain the differences between both compilation strategies.
After reading this post, you will know about what Java compilers do, the differences between different compiling approaches, and when it is more appropriate using an AOT compiler.
...
Compiling a program means **transforming source code** from a high-level programming language, such as Java or Python, into a low-level [machine code](https://en.wikipedia.org/wiki/Machine_code).
Machine code are low-level instructions tailored to execute in a particular microprocessor.
Compilers are programs designed to perform this transformation efficiently.
The goal of a compiler is to create a consistent executable of the compiled program.
A consistent executable is one that is attained to the specification written in the source code, runs fast, and it is safe.
...
- The **C1 compiler** is a fast, lightly optimizing bytecode compiler that performs some value numbering, inlining, and class analysis.
  It uses a simple CFG-oriented SSA "high" IR, a machine-oriented "low" IR, a linear scan register allocation, and a template-style code generator.

- The **C2 compiler** is a highly optimizing bytecode compiler that uses a "sea of nodes" SSA "ideal" IR, which lowers to a machine-specific IR of the same kind.
  It has a graph-coloring register allocator; colors all machine states, including local, global, and argument registers and stack. Optimizations in the C2 compiler include global value numbering, conditional constant type propagation, constant folding, global code motion, algebraic identities, method inlining (aggressive, optimistic, and/or multi-morphic), intrinsic replacement, loop transformations (unswitching, unrolling), array range check elimination, and others.
...
The latter generates machine code **before the execution** of the program itself (i.e., during the bytecode verification and build phase of the application).
...
<figure class="jb_picture">
  ...
  <figcaption class="stroke"> 
    Java source code is first compiled to bytecode, and subsequently interpreted or executed as native code. Heavy optimizations are reserved for the JIT-compilation phase. <a href="https://dl.acm.org/doi/10.1145/3067695.3082521">Source</a>.
  </figcaption>
</figure>

To transform JVM bytecode into machine code that is executable in a specific hardware architecture, the JVM **interprets the bytecode at runtime**.
...
Thanks to the runtime information, JIT compilers perform much more sophisticated optimizations than the `javac` compiler to improve performance.
...
Despite the advances in JIT compilers, Java applications are still a lot slower than other languages such as C or Rust, which produce native code.
...
[AOT](https://en.wikipedia.org/wiki/Ahead-of-time_compilation) is a form of static compilation that consists in transforming the program into a machine code **before it is executed**.
This is the "old-fashioned" way in which the code in programming languages such as C is statically linked and compiled.
The machine code obtained as a result is tailored to a specific operating system and architecture, facilitating a very fast execution. 

The [GraalVM](https://github.com/graalvm/graal.git) compiler is capable of performing highly optimized AOT compilation of JVM bytecode.
GraalVM is written in Java and uses JVMCI[^1] to integrate with the Hotspot VM.
The focus GraalVM is in offering high performance and extensibility of the compiled application.
This means that it executes faster with less overhead, which translates into optimal resource consumption with less CPU and memory usage that the traditional JIT compiler shipped with the JVM.
...
The following figure illustrates the AOT compilation process in the GraalVM compiler using the [native-image](https://www.graalvm.org/22.0/reference-manual/native-image/) technology.
It receives as input all classes from the application, libraries, the JDK, and the Java Virtual Machine.
Then an iterative analysis using points-to analysis is performed until fixed point is reached.
...
The class data of the initialised classes is loaded into the image heap which then, in turn, gets saved into standalone executable (into the text section).
...
{% responsive_image path: img/posts/2022/native_image_creation_process.png alt:"Native image creation process in Quarkus." %}
  <figcaption class="stroke"> 
    Native image creation process in GraalVM.
  </figcaption>
</figure>
...
A major advantage is that the executable does not require having the JVM installed in the client machine to run.
This makes programming languages that compile to JVM bytecode as fast as languages such as C, C++, Rust, or Go which are used for high peforming computing.[^2]
...
They offer faster startup speed, which results in shorter boot time and instant scale-up in the case of microservices running on Docker containers.
...
The low memory consumption allows running more containers with the same RAM, reducing the costs of cloud providers.
...
<figure class="jb_picture">
  ...
  <figcaption class="stroke">
    AOT vs. JIT. <a href="https://twitter.com/thomaswue/status/1145603781108928513?s=20&t=-6ufSBjc46mfN5d_6Y2-Rg">Source</a>. 
  </figcaption>
</figure>
...
Dynamic language capabilities such as Java Native Interface (JNI), Java Reflection, Dynamic Proxy objects (`java.lang.reflect.Proxy`), or classpath resources (`Class.getResource`) are not supported.
...
It is worth mentioning that the closed work assumption is good for security as it eliminates the possibility of various code injections (e.g., the [Log4j vulnerability](https://nvd.nist.gov/vuln/detail/CVE-2021-44228) that shocked the web in 2021 was possible due to the exploitation of a dynamic class loading in Java).
On the other hand, the points-to-analysis makes AOT compilation slower than JIT because the 
...
However, since the native image creation analyzes the bytecode in the application **and all its dependencies**, there is a risk of violating the close world if at least one of the dependencies relies on some dynamic Java.
...
Therefore, the technology still needs some time to mature before its massive adoption.
...
It is possible compiling JVM bytecode using either AOT or JIT approaches. It would be wrong to say one approach is better than the other since they are suited for different situations. The GraalVM compiler allows for building high-performance applications with AOT compilation, reducing the startup time and improving the performance considerably. This power comes at the cost of complying with the close world assumption (no Java dynamic features are allowed). Developers can still use the standard JIT compiler in the Hotspot VM to use dynamic features, which supports machine code generation at runtime.
